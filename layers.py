...
class ConvolutionalLayer(LayerWithParameters):
    ...
    def fprop(self, inputs):
        ...
        if output_height < 1 or output_width < 1:
            raise Exception('Measurments do not produce valid output')

        outputs = np.zeros((batch_size, self.num_output_channels, output_height, output_width))

        for batch in np.arange(batch_size):
            for output_channel in np.arange(self.num_output_channels):
                for input_channel in np.arange(self.num_input_channels):
                        outputs[batch, output_channel] += convolve2d(inputs[batch, input_channel], self.kernels[output_channel, input_channel], mode='valid')
                outputs[batch, output_channel] += self.biases[output_channel]
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        for batch in np.arange(batch_size):
            for input_channel in np.arange(self.num_input_channels):
                for output_channel in np.arange(self.num_output_channels):                  
                    grads_wrt_inputs[batch, input_channel] += convolve2d(grads_wrt_outputs[batch, output_channel], kernels_flipped[output_channel, input_channel])
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        grads_wrt_kernels = np.zeros((batch_size, self.num_input_channels, self.kernel_height, self.kernel_width))
        grads_wrt_biases = np.zeros(self.num_output_channels)

        inputs_flipped = inputs[:, :, ::-1, ::-1]

        for output_channel in np.arange(self.num_output_channels):
            grads_wrt_biases[output_channel] = np.sum(grads_wrt_outputs[:, output_channel])
            for input_channel in np.arange(self.num_input_channels):
                for batch in np.arange(batch_size):                  
                    grads_wrt_kernels[output_channel, input_channel] += convolve2d(grads_wrt_outputs[batch, output_channel], inputs_flipped[batch, input_channel], mode='valid')
...
class MaxPooling2DLayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        if ((self.input_height - self.size) % self.stride != 0) or ((self.input_width - self.size) % self.stride != 0):
            raise Exception('Measurments do not produce valid output')

        pooled_input_height = int((self.input_height - self.size)/self.stride + 1)
        pooled_input_width = int((self.input_width - self.size)/self.stride + 1)

        pooled_inputs = np.zeros((batch_size, num_input_channels, pooled_input_height, pooled_input_width))

        for b in np.arange(batch_size):
            for c in np.arange(num_input_channels):
                st_h = 0
                for h in np.arange(pooled_input_height):
                    st_w = 0
                    for w in np.arange(pooled_input_width):
                        pooled_inputs[b,c,h,w] = np.max(inputs[b, c, st_h:st_h+self.size, st_w:st_w+self.size])
                        st_w += self.stride
                    st_h += self.stride
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        output_height = outputs.shape[2]
        output_width = outputs.shape[3]

        inputs_mask = np.zeros(inputs.shape)
        grads_wrt_inputs = np.zeros(inputs.shape)

        for b in np.arange(batch_size):
            for c in np.arange(num_input_channels):
                h = 0
                while h < self.input_height:
                    w = 0
                    while w < self.input_width:
                        inputs_mask[b,c, h:h+self.size, w:w+self.size] = (inputs[b,c, h:h+self.size, w:w+self.size] == np.max(inputs[b,c, h:h+self.size, w:w+self.size])).astype('int64')                        
                        w += self.stride
                    h += self.stride

        for b in np.arange(batch_size):
            for c in np.arange(num_output_channels):
                input_h = 0
                for h in np.arange(output_height):
                    input_w = 0
                    for w in np.arange(output_width):                        
                        grads_wrt_inputs[b,c, input_h:input_h+self.size, input_w:input_w+self.size] = grads_wrt_outputs[b,c,h,w] * inputs_mask[b,c, input_h:input_h+self.size, input_w:input_w+self.size]                        
                        input_w += self.stride
                    input_h += self.stride

        #grads_wrt_inputs = np.pad(grads_wrt_outputs, ((0,0), (0,0), (2,2), (2,2)), mode='constant') * input_mask
